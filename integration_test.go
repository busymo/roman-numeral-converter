...
func TestConvertRangeErrors(t *testing.T) {
    ...
    assert.Contains(t, rr.Body.String(), "Invalid input. Ensure input is in the format 'from-to'.")
    ...
    r.ServeHTTP(rr, req)
    assert.Equal(t, http.StatusBadRequest, rr.Code)
    assert.Contains(t, rr.Body.String(), "Invalid input. Ensure numbers are between 1 and 3999 and 'from' is less than or equal to 'to'.")
